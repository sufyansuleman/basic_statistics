---
title: "R Packages and Libraries"
---
...
## When Do You Use This? {#sec-packages-libraries .callout-tip}
...
### Exercise 1 (Guided): Install, Load, Explore

1. Install the `palmerpenguins` package (if not already installed): `install.packages("palmerpenguins")`.
2. Load it with `library(palmerpenguins)`.
3. Run `?penguins` to open the help page. What are the column names and how many rows does the dataset have?
4. Run `packageVersion("palmerpenguins")`. What version is installed?
5. Run `citation("palmerpenguins")` and write out the full citation.
...
### Exercise 2 (Semi-guided): Navigate Package Documentation

Pick any package you have installed (e.g., `survival`, `lme4`, or `broom`).

1. Open the package index with `help(package = "survival")`. How many exported functions does it list?
2. Find the help page for one function you haven't used before. What does it do? What are its main arguments?
3. Check whether there is a vignette: `vignette(package = "survival")`. Open one with `vignette("timereg")` or another listed vignette.
4. Use `::` to call a function from the package without loading it: e.g., `survival::Surv(1:5, c(1,0,1,1,0))`. What does the output look like?

### Exercise 3 (Open-ended)

Find a package relevant to your own research area that you have not used before.

1. Search for it on CRAN (`https://cran.r-project.org`) or Bioconductor.
2. Install it, load it, and explore its documentation (help index + at least one vignette if available).
3. Run a minimal example from the documentation.
4. Write two sentences describing what the package does and whether it would be useful for your work.
...
### Citing R Packages in Your Work

::: {.callout-note}
**In a methods section:** "All analyses were performed in R (version X.X.X; R Core Team, YEAR). Statistical analyses used the `tidyverse` (Wickham et al., YEAR), `survival` (Therneau, YEAR), and `lme4` (Bates et al., YEAR) packages."

**How to get a citation for any package:** Run `citation("packagename")` in R and copy the output into your reference manager.

**What to always include:**

- R version (`R.version.string`)
- All key packages with version numbers (`packageVersion("pkg")`) and literature citations
- RStudio version if relevant (`rstudioapi::versionInfo()`)

**Tip:** The `grateful` package can automatically generate a bibliography of all R packages used in a project:

```r
install.packages("grateful")
grateful::cite_packages(output = "file", out.dir = ".")
```
:::
