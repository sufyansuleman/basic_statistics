...
diag_df |>
  arrange(desc(.cooksd)) |>
  select(.rownames, albumin, log_bili, .fitted, .resid, .cooksd, .hat) |>
  head(10)
```
...
pbc2 <- pbc |>
  mutate(
    died      = as.integer(survival::pbc$status == 2),
    sex       = factor(survival::pbc$sex, levels = c("f", "m"), labels = c("Female", "Male"))
  ) |>
  filter(!is.na(died))
...
augment(fit_ex) |>
  arrange(desc(.cooksd)) |>
  select(.rownames, albumin, log_bili, .resid, .cooksd) |>
  head(5)
