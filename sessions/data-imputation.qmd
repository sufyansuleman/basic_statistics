# Missing Data and Imputation {#sec-imputation}
...
> Your PBC dataset has missing platelet counts for ~11% of patients. If you run a complete-case analysis, those patients are silently dropped. But are the missing values random? If sicker patients are more likely to have missing platelets, dropping them biases your estimate. Missing data methods let you use all available information and be transparent about uncertainty.
...
bind_rows(
  ...
  summary(pooled, conf.int = TRUE) |>
    as_tibble() |>
    rename(estimate = estimate, conf.low = `2.5 %`, conf.high = `97.5 %`) |>
...
pbc |>
  ...
  glm(platelet_miss ~ log(bili), data = _, family = binomial) |>
