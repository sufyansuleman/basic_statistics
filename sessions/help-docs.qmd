...
This course covers a set of core statistical methods used in real research data analysis. The following table maps each method to the session where it is taught and practiced.

| Method | Session | Notes |
|---|---|---|
| Descriptive statistics | `basic-statistics.qmd` | summaries, distributions, and data description |
| Data transformation / standardization | `basic-statistics.qmd` | log transform, scaling, and normalizing data |
| Confidence intervals | `basic-statistics.qmd` | CIs for means, proportions, and regression estimates |
| t-tests | `basic-statistics.qmd` | one-sample, two-sample, paired, and Welch tests |
| Power / sample size calculation | `basic-statistics.qmd` | power analysis for mean differences and proportions |
| Correlation | `clinical-research-methods.qmd` | Pearson and Spearman correlation analysis |
| Linear regression | `basic-statistics.qmd` / `advanced-visualization.qmd` | regression modeling, slope interpretation, and diagnostics |
| ANOVA | `clinical-research-methods.qmd` | one-way and two-way ANOVA, comparison of group means |
| Multiple comparisons | `clinical-research-methods.qmd` | Tukey, Dunnett, Bonferroni, Holm adjustments |
| Nonparametric tests | `clinical-research-methods.qmd` | Wilcoxon test and rank-based comparisons |
| Chi-square tests | `clinical-research-methods.qmd` | categorical data association and contingency tables |
| Logistic regression | `clinical-research-methods.qmd` | binary outcome modeling and odds ratio interpretation |
| Survival analysis | `survival-and-time-to-event.qmd` | Kaplan-Meier curves, log-rank tests, Cox regression |
| Model diagnostics / interpretation | `advanced-visualization.qmd` / `clinical-research-methods.qmd` | residuals, QQ plots, assumption checks, report-ready interpretation |
...
- R reference and documentation: @R-Core-Team-2023; @CRAN.
- Community Q&A and examples: @StackOverflow-R.

## Resources

- Quick links: CRAN (https://cran.r-project.org), RDocumentation, and Stack Overflow. See the course README and `data_sources.md` for course-specific pointers and recommended datasets.
