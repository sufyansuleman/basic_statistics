...
- `survival::pbc`: clinical dataset (primary biliary cirrhosis) with labs and demographics; useful for demonstrating distributional summaries and missingness.
- Local fallback: `data/scd_simulated_data.rda` (simulated SCD data) if present in `data/` — useful when a small in-repo example is preferred.
...
# Re-create a minimal workflow using `survival::pbc` or local data (see `data_sources.md`)
library(dplyr)
library(ggplot2)
...
- Example scripts: moved from `dev_code_data` — use inline examples above or see `data_sources.md` for public dataset load snippets.
