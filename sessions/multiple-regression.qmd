...
- **stageStage 2 (−0.09):** Stage 2 patients have 0.09 g/dL lower albumin than Stage 1 patients, after adjusting for bilirubin and prothrombin time.
...
| Multiple `lm(albumin ~ log_bili + log_proto + stage)` | −0.22 | Effect of bilirubin, *adjusted for* the other predictors |

The coefficient shrank from −0.34 to −0.22. Prothrombin time and stage were partially *confounding* the bilirubin-albumin relationship. The multivariable model gives the purer estimate.
...
It means Stage 2 patients have 0.09 g/dL lower albumin than Stage 1 patients *after adjusting for other predictors.* Change the reference category and the coefficient changes. Always state the reference group when reporting factor coefficients.
:::
